#!/bin/bash
# ============================================================================
# MSRT inference launcher (CUDA GPU + Accelerate + DeepSpeed)
# 与 infer_moe_npu.sh 的差异：
#   - 用 CUDA_VISIBLE_DEVICES 选择 GPU，不依赖 Ascend / torch-npu；
#   - 设置 MSRT_BACKEND=cuda，infer_moe.py 据此跳过 torch_npu 导入。
# 分布式配置与 NPU 统一使用 acceleate_config.yaml（DEEPSPEED + bf16）。
# GPU 依赖见 requirements_gpu.txt。
# ============================================================================
...
# ---------------------------------------------------------------------------
echo "============================================"
echo " MSRT MoE Inference (CUDA + Accelerate)"
echo "============================================"
echo " Data:       ${DATA_PATH}"
